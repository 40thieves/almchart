(function(root, factory) {
	...
	AlmChart.prototype.x = function(data) { return data.sources[this.config.dataSourceNames.x]; };
	AlmChart.prototype.y = function(data) { return data.sources[this.config.dataSourceNames.y]; };
	...
	AlmChart.prototype.fetchData = function() {
		d3.json(this.config.url, this.draw.bind(this));
	};

	AlmChart.prototype.draw = function(err, data) {
		var self = this;
	...
	AlmChart.prototype.calculateRanges = function() {
		var self = this;
		...
		this.ranges.lastYear = d3.max(this.data, function(d) {
			...
			var largestY = d3.max(self.x(d), function(y) {
				return y[0];
			});
		...
		this.ranges.maxX = d3.max(this.data, function(d) {
			return d3.max(self.x(d), function(s) {
				return s[1];
			});
		});
		...
		this.ranges.maxY = d3.max(this.data, function(d) {
			return d3.max(self.y(d), function(s) {
				return s[1];
			});
		});
	...
	AlmChart.prototype.drawYearLabel = function() {
		this.yearLabel = {};
	...
	AlmChart.prototype.setDomains = function() {
	...
	AlmChart.prototype.bisect = d3.bisector(function(d) {
		// Year is first elem in array - uses as selector
		return d[0];
	});

	AlmChart.prototype.appendAxes = function() {
		// Append x axis
		this.chart.append('g')
			.attr('class', 'x axis')
	...
	AlmChart.prototype.drawCircles = function() {
		var self = this;
		...
		this.circle = this.chart.append('g')
			...
			.enter()
				...
				.attr('class', 'circle')
				.call(this.position, this) // Set position data for the circle
		...
		this.chart.transition()
			...
			.ease('linear')
			.tween('year', this.tweenYear.bind(this)) // Values for each year are interpolated
	 ...
	 */
	AlmChart.prototype.position = function(circle, self) {
	 ...
	 */
	AlmChart.prototype.tweenYear = function() {
		var self = this;
	...
	AlmChart.prototype.displayYear = function(year) {
		...
		this.circle.data(this.interpolateData(year), self.key) // Interpolates data
			.call(this.position, this) // Re-positions circles using interpolated data
	...
	AlmChart.prototype.interpolateData = function(year) {
		...
		return this.data.map(function(d) {
			return {
				...
				url: d.url,
				sources: {
					counter: self.interpolateValues(self.x(d), year), // Interpolates for x dimension
					twitter: self.interpolateValues(self.y(d), year)  // Interpolates for y dimension
				}
			};
		});
	};
	 ...
	 */
	AlmChart.prototype.interpolateValues = function(values, year) {
